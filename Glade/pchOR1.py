...
#	    l (length of gate), nf (number of gate fingers), polyCont (poly-metal1 contact),
#       leftCnt (contacts on left of mos), rightCnt (contacts on right of mos).
#
...
def pchOR1(cv, w=2e-6, l=1e-6, nf=1, polyCnt=0, leftCnt=1, rightCnt=1) :
	...
	if poly_ovlp_cut <= 0 :
		...
		return
	pplus_ovlp_active = tech.getLayerEnc("Parea", "drawing", "DIFF", "drawing")
	...
	# Create nwell
	nwell = tech.getLayerNum("NWL", "drawing")
	...
	# Create pplus
	pimp = tech.getLayerNum("Parea", "drawing")
